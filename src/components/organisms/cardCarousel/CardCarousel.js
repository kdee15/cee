...
import CardCarouselCard from "../../molecules//cardCarouselCard/CardCarouselCard";
import classes from "./CardCarousel.module.scss";

export default function CardCarousel({ contentModule }) {
  const settingsCardCarousel = {
    dots: false,
    infinite: true,
    speed: 2000,
    slidesToShow: 1,
  ...
  return (
    <div className={`${classes.oCardCarousel}`}>
      <Slider {...settingsCardCarousel}>
        {contentModule.fields.cards?.map((card, index) => {
          return <CardCarouselCard key={index} {...card} />;
        })}
      </Slider>
      <style global jsx>{`
        .slick-slider {
